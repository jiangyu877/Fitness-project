...
本文定义数据结构边界、完整性原则和迁移流程，不定义筛查题、风险阈值、热量/训练规则、调整算法或留存期限。下列模型为逻辑 ERD，不是可执行迁移。
...
| 计划 | `plan` 标识用户计划序列；`plan_version` 不可变；同用户同日期最多一个 `ACTIVE` 版本 |
| 审核 | 营养、训练分开唯一；决定、审核人角色、时间和所审版本不可缺失 |
| 用户确认 | 饮食、训练分开唯一；拒绝必须引用原因代码；确认只针对精确版本 |
| 内容 | demo、待审核、批准、停用分离；真人发布事务校验所有引用均可发布 |
| 执行记录 | 日期和任务唯一；训练组使用稳定 ID 和顺序；幂等键防止重复写入 |
| 周调整 | 事实、数据周期、可信度、变更和保持变量归属同一不可变版本 |
| 待办 | 只保存领域资源引用和处理状态，不复制计划/风险事实 |
| 审计/outbox | 只追加；业务变更、审计和 outbox 同一事务提交 |

需要产品冻结后才能转成检查约束的内容：有效状态集合、转换边界、风险暂停范围、审核人员分离规则、确认期限、留存/匿名化规则。
...
删除/匿名化不得先于产品留存矩阵实现。矩阵必须覆盖在线表、审计、导出、日志、快照和时间点恢复备份；批准前只设计接口和数据定位能力，不执行真实删除策略。

## 10. 迁移开工门槛

在产品冻结状态机、权限矩阵、专业规则版本字段和留存矩阵前，不创建可执行 schema 或迁移。允许先评审本逻辑 ERD、命名、事务边界和迁移机制。
