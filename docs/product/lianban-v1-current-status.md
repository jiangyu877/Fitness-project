...
- 2026-09-18 G2-M2a（`G2-M2A-LOCAL-WEEKLY-FEEDBACK`）收口：本地运行时新增周反馈面（`GET/POST /p11-local/weekly-feedback`，P12 视图信封 + 提交置 `ADJUSTMENT_PENDING`）与 H5 周反馈页（复用 P12 UI 模块，严格 fail-closed 解析）——focused `16/16`、typecheck/build `EXIT=0`；`BROWSER`：真实 Edge 窗口渲染周反馈页（结构字段表单），证据 `docs/engineering/evidence/p11-local-weekly-feedback-2026-09-18.png`；全量单 worker `74/74 files`、`848/848 tests`、`EXIT=0`，临时库 0；`SELF_REVIEW_COMPLETE`（ZCode 自审，产品负责人验收）。
- 2026-09-18 G2-M2b（`G2-M2B-LOCAL-WEEKLY-ADJUSTMENT`）收口：本地运行时把"周反馈提交 → 周调整版本 → 双审核"跑通为真实路由闭环（PGlite 侧补种员工/授权/建档/计划 + test-only 计划时钟 + `GET /p11-local/adjustment`）——focused `8/8`、运行时+启动器 `15/15`、typecheck/build `EXIT=0`、全量单 worker `74/74 files`、`849/849 tests`；**两处设计发现如实记录**：① `professionalRulesApproved` 是计划机制总闸（运行时以测试夹具置为已批准，与 P08 规格一致）；② 演示内容不可发布 → 调整版本稳定停在 `READY_TO_PUBLISH`（`DEMO_CONTENT_REFERENCED`），基线保持 ACTIVE；`SELF_REVIEW_COMPLETE`（ZCode 自审，产品负责人验收）。
...
- 最近活动场景 `G2-M2B-LOCAL-WEEKLY-ADJUSTMENT`（M2 第二个实现切片）已于 2026-09-18 按授权范围执行完毕并停止（`SELF_REVIEW_COMPLETE`，ZCode 自审 + 产品负责人验收）；**M1 与 M2 已完成**（任务生成/今日任务视图/周反馈面/周调整链路至发布门禁）；下一候选为 M3（双 persona 全链路浏览器 E2E + 分层证据 + G2 判定）。
