...
describe('ReviewDetailPage', () => {
  it.each([
    ...
    render(React.createElement(ReviewDetailPage, { kind }));

    expect(screen.getByRole('heading', { name: title })).toBeInTheDocument();
    expect(screen.getByText('林晓雨')).toBeInTheDocument();
    expect(screen.getByText('计划版本 V2')).toBeInTheDocument();
  ...
  it('keeps approval and return decisions local while preserving the safety warning', () => {
    render(React.createElement(ReviewDetailPage, { kind: 'diet' }));
